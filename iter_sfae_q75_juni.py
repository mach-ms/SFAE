...
import pm1_simulator as sim     #+-1最优嵌入模拟器| +-1 embedding simulator
import tf_dct_module as tdm     #可导的idct操作| differentiable idct
import def_extractor            #定义了nndctr| definition of nndctr
...
def adjust_rhos(rho_p1, rho_m1, adv_grad):
    adj_rho_p1 = rho_p1.copy()
    adj_rho_m1 = rho_m1.copy()
    adj_rho_p1[adv_grad > 0] = rho_p1[adv_grad > 0] * MU
    adj_rho_m1[adv_grad > 0] = rho_m1[adv_grad > 0] / MU
    adj_rho_p1[adv_grad < 0] = rho_p1[adv_grad < 0] / MU
    adj_rho_m1[adv_grad < 0] = rho_m1[adv_grad < 0] * MU
...
def batch_gen(
    ...
    #==========================================
    # 定义特征提取器| definition of nndctr feature extractor
    ...
    with tf.Session(config = config) as sess:
        ...
        for img_idx in range(img_num):
            ...
            # 生成不同的stego| generator the stego candidate
            ...
            for k_idx in range(itvl_num - 1):
                ...
                # 嵌入第一部分| embed first part
                ste1 = sim.embed_part1(
                    cov     = cov, 
                    rho_p1  = rho_p1, 
                    rho_m1  = rho_m1, 
                    pos     = pos_nzac, 
                ...
                dct_buf[0, :, :, 0] = ste1.astype(np.float32)
                result = sess.run(
                    fetches = fea_nndctr, 
                    feed_dict = {ph_dct: dct_buf})
                ...
                # 调整代价，嵌入第二部分| adjust rho, embed second part
                adv_grad = np.reshape(result[0], (256, 256))
                adj_rho_p1, adj_rho_m1 = adjust_rhos(rho_p1, rho_m1, adv_grad)
                ste2 = sim.embed_part2(
                    cov     = ste1, 
                    rho_p1  = adj_rho_p1, 
                    rho_m1  = adj_rho_m1, 
                    pos     = pos_nzac, 
                    ...
                    krate   = krate, 
                    seed    = img_idx)
                ste_img_candi[k_idx, :, :] = ste2

                # 计算stego的feature| compute the feature of stego
                dct_buf[0, :, :, 0] = ste2.astype(np.float32)
                result = sess.run(
                    fetches = fea_nndctr, 
                    feed_dict = {ph_dct: dct_buf})
                s_fea = np.reshape(result[0], (1, 8000))
            ...
            # get the best stego
            kmax = dists.argmin(axis = 0)
            ste = ste_img_candi[kmax, :, :]
